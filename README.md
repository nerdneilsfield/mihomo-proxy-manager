...
- 状态页提供 HTML dashboard，`{status_path}/api` 提供 JSON API。
...
| `access_log.status.*` | 状态页访问统计控制：IP 遮蔽、recent rows、top list 长度。 |
...
| `surfboard` | 最小 full profile | `-nodes` | Surfboard | 为客户端兼容性输出 `ss`、`trojan`、`vmess`；`hysteria2` 和 `vless` 会跳过。 |
...
状态页只展示聚合统计，永不展示完整 `headers_json`。访问统计暴露在 `status_path` HTML 和 `{status_path}/api` JSON：总事件数、retention、Top IPs、Top User-Agents、Top Headers、Top Paths，以及可选 recent rows。状态页默认遮蔽 IP，默认不显示 recent rows；recent rows 也不暴露完整 headers。保持 `status_path` 高熵且不公开；不要把高熵私有 header 放进 `access_log.headers.stats_allowlist`。
